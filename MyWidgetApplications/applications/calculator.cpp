...
Calculator::Calculator(int XX, int YY) : Application(XX,YY){
    ...
    // VIEW -----
    // display
    _op1W = new StaticText(25, 2, "0");
    _widgets.push_back(_op1W);

    _op2W = new NumberEditor(25, 25, 100, 25, true, _lbound, _ubound);
    _widgets.push_back(_op2W);

    _operW = new StaticText(9, 25, "");
    _widgets.push_back(_operW);

    // numbers
    for(int i=1; i<10; i++){
        _numberWs[i] = new FuncButton(25+(i-1)%3*50, 50+(i-1)/3*50, Widget::to_str(i), [=](){this->number_pushed(i);}, 50,50);
        _widgets.push_back(_numberWs[i]);
    }
    _signW = new FuncButton(25, 50+3*50, "+/-", [&](){_op2 = _op2W->get_number(); _op2 = -_op2; update();}, 50,50);
    _widgets.push_back(_signW);
    _numberWs[0] = new FuncButton(25+1*50, 50+3*50, "0", [=](){this->number_pushed(0);}, 50,50);
    _widgets.push_back(_numberWs[0]);

    // extra
    _backspaceW = new FuncButton(25+2*50, 25, "<X", [=](){_op2 = _op2W->get_number(); _op2 /= 10; update();}, 50,25);
    _widgets.push_back(_backspaceW);
    _undoW = new FuncButton(30+3*50, 25, "undo", [=](){this->undo();}, 50,25);
    _widgets.push_back(_undoW);
    _newW = new FuncButton(30+4*50, 25, "new", [=](){this->reset();}, 50,25);
    _widgets.push_back(_newW);

    // operations
    _addW = new FuncButton(30+3*50, 50, "+", [=](){this->operation_pushed(Operation::ADD);}, 50,50);
    _widgets.push_back(_addW);
    _substractW = new FuncButton(30+4*50, 50, "-", [=](){this->operation_pushed(Operation::SUBSTRACT);}, 50,50);
    _widgets.push_back(_substractW);
    _multiplyW = new FuncButton(30+3*50, 2*50, "*", [=](){this->operation_pushed(Operation::MULTIPLY);}, 50,50);
    _widgets.push_back(_multiplyW);
    _divideW = new FuncButton(30+4*50, 2*50, "/", [=](){this->operation_pushed(Operation::DIVIDE);}, 50,50);
    _widgets.push_back(_divideW);
    _equalsW = new FuncButton(30+3*50, 3*50, "=", [=](){this->operation_pushed(Operation::EQUALS);}, 100,50);
...
void Calculator::number_pushed(int n){
    ...
    if (0 <= _op2)
        ...
        _op2 = 10*_op2 - n;
    if(_ubound < _op2) _op2 = prev_op2;
    if(_lbound > _op2) _op2 = prev_op2;
    update();
}
